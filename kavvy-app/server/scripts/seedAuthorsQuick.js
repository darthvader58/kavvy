...
const MAX_AUTHORS = Infinity; // Seed all authors from CSV
...
async function seedAuthors() {
  try {
    ...
    console.log(`📚 Found ${authorMap.size} unique authors, seeding first ${MAX_AUTHORS}...`);
    ...
    for (const [authorName, authorData] of authorMap) {
      if (authorCount >= MAX_AUTHORS) break;
      ...
      try {
        ...
        if (authorCount % 50 === 0) {
          console.log(`   Progress: ${authorCount}/${MAX_AUTHORS} authors...`);
        }
      } catch (error) {
        if (error.code === 11000) {
          skippedCount++;
